...
export default function FloatingContact() {
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-[70] grid grid-cols-3 gap-1.5 border-t border-slate-200 bg-white p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] shadow-[0_-6px_20px_rgba(11,31,63,0.14)] md:hidden">
        <a
        ...
        <button
          ...
          className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] px-2 py-2 text-xs font-extrabold text-white cursor-pointer active:scale-95 transition-transform"
        ...
        <a
          ...
          onClick={() => trackEvent("email_click", { source: "floating_mobile", email: siteConfig.email })}
          className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-navy hover:bg-navy-dark px-2 py-2 text-xs font-extrabold text-white active:scale-95 transition-transform"
          aria-label={`Email ACS: ${siteConfig.email}`}
        >
      ...
      <div className="hidden md:flex fixed bottom-6 right-6 z-[60] flex-col gap-3.5 items-end">
        ...
        <div className="relative group flex items-center">
          ...
          <a
            href={`mailto:${siteConfig.email}`}
            onClick={() => trackEvent("email_click", { source: "floating_desktop", email: siteConfig.email })}
            className="w-13 h-13 sm:w-14 sm:h-14 bg-sky-600 hover:bg-sky-700 text-white rounded-full flex items-center justify-center shadow-[0_8px_25px_rgba(14,165,233,0.35)] hover:-translate-y-1 hover:shadow-[0_12px_30px_rgba(14,165,233,0.5)] border border-white/20 transition-all duration-200 cursor-pointer"
            aria-label={`Email ACS: ${siteConfig.email}`}
          >
